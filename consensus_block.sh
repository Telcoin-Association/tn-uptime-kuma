#!/bin/bash
# Consensus Block Progress Monitor
# Queries all 5 nodes individually
# Alerts if any node's commit_timestamp is older than 60 seconds

PUSH_URL="https://kuma.telscan.xyz/api/push/UH7ebacL2x"
THRESHOLD=60

NODES=(
    "node1|https://node1.telcoin.network"
    "node2|https://node2.telcoin.network"
    "node3|https://node3.telcoin.network"
    "node4|https://node4.telcoin.network"
    "node5|https://node5.telcoin.network"
)
...
if [ "$STATUS" == "down" ]; then
    MSG="Consensus issue:$STALE |$DETAILS"
else
    MSG="All nodes OK —$DETAILS"
fi
